  ...
  const fetchData = async () => {
    try {
      ...
      if (res?.data.dataResult && res.data.dataResult.length > 0) {
        const portData = res.data.dataResult.find((d: any) => d.port === 2);
  ...
  if (loading) {
    return (
      <div className="action-loading">
        ...
        <span className="action-loading-text">Loading Port 2 data...</span>
      </div>
    );
  }
