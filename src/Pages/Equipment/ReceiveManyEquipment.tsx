...
export default function ReceiveManyEquipment() {
  const [formData, setFormData] = useState<Receive>({ equipmentSerialNo: "", orderID: "" });
  ...
  const [orders, setOrders] = useState<Receive[]>(() => {
    const saved = localStorage.getItem("receiveOrders");
  ...
  useEffect(() => {
    localStorage.setItem("receiveOrders", JSON.stringify(orders));
  }, [orders]);
  ...
  const handleAdd = (data: Receive) => {
    if (!data.equipmentSerialNo) return;

    const newItem: Receive = {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.data?.isSuccess) {
        ...
        setOrders([]);
        equipmentRef.current?.focus();
        localStorage.removeItem("receiveOrders");
      } else {
  ...
  const handleCancel = () => {
    ...
    equipmentRef.current?.focus();
    localStorage.removeItem("receiveOrders"); // ล้าง localStorage
  };
  ...
  return (
    <>
      <AppHearder title="รับหลายเครื่อง" />
      <Box
        sx={{
          display: "flex",
